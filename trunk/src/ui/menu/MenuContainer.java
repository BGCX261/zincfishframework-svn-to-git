...
public class MenuContainer extends UPanel implements IKeyInputHandler {
	...
	private MCascadeMenuItem generateMenuImpl(MenuData menu) {
		if (menu == null)
			return null;
		MCascadeMenuItem item = new MCascadeMenuItem(menu.text, menu.onClick,
				null, null, Config.PLAIN_SMALL_FONT);
		if (menu.hasSubMemu()) {
			MCascadeMenuItem[] items = new MCascadeMenuItem[menu.getSubMemu()
					.size()];
			for (int i = 0; i < items.length; i++) {
				MenuData menuItemData = (MenuData) menu.getSubMemu().get(i);
				items[i] = generateMenuImpl(menuItemData);
				menuItemData = null;
			}
			item.initChildItem(items);
			item.setToken(Resources.TOKEN);
			items = null;
		}
		return item;
	}

	public void generateMenu(MenuData menu) {
		MCascadeMenuItem root = generateMenuImpl(menu);
		setRoot(root);
		root = null;
	}
	...
	public boolean keyPressed(int iKeyCode) {
		...
		if (iKeyCode == CSDevice.NK_LSOFT && (!isOpened)) { // �����
			...
			cel.componentEventFired(this, 2, null, 0);
		} else if (isOpened) {
			isAction = true;
			if (iKeyCode == CSDevice.NK_LSOFT) {
				...
				case CSDevice.KEY_FIRE:
					if (cel != null) {
						// //System.out.println("-----Menu---start onfire--------------");
						if (nowNode.getSubMenu().getSelectedItem()
								.getOperable()) { // �ɲ���
							closeAll();
							isOpened = false;
							// //System.out.println("-----Menu---clicked onfire--------------");
						...
						// //System.out.println("-----Menu---End onfire--------------");
					}
					break;
	...
	protected boolean animate() {
		...
		if (nowNode != null && nowNode.getSubMenu() != null) {
