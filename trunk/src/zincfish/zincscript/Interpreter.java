...
import zincfish.zinclib.AbstractLib;
import zincfish.zinclib.DOMLib;
import zincfish.zinclib.NetLib;
import zincfish.zinclib.StdLib;
import zincfish.zinclib.StrLib;
...
public class Interpreter {
	...
	private boolean isDirect = false;// 标示解释执行引擎是否处于直接解释状态
	...
	public Object interprete(int from, int to) throws ZSException {
		...
		// //System.out.println("interprete====line======" + currentLine
		// + " from = " + from + " to = " + to);
		if (currentLine == null)
			return null;
		// FIXME 发布的时候可以移除下面这段代码
	...
	private void interpreteFunctionCall() throws ZSException {
		...
		Object o = interpreteCallFunction(functionName);
		if (isDirect)
			returnValue = o;
		o = null;
		getNextToken();
	...
	public Object callFunction(String name, ArrayList params)
		...
		if (funcs.containsKey(name)) {
			...
			value = p.interprete(func.startLine + 1, func.endLine - 1);
	...
	private Object callLibFunction(String name, ArrayList params)
			throws ZSException {
		AbstractLib lib = null;
		if (name.startsWith("_zss"))
			lib = StdLib.getInstance();
		else if (name.startsWith("_zsn"))
			lib = NetLib.getInstance();
		else if (name.startsWith("_zsd"))
			lib = DOMLib.getInstance();
		else if (name.startsWith("_zsr"))
			lib = StrLib.getInstance();
		...
		if (lib != null) {
			...
			throw new ZSException("库函数" + name + "不存在");
	}
	...
	private Object interpreteCallFunction(String functionName)
		...
		do {
			...
			Object param = interpreteExpression();
			params.add(param);
		} while (parser.type == ',');

		return scriptEngine.callFunction(functionName, params);
	}

	// 函数定义
	private void defineFunction() throws ZSException {
		FuncEntry fDef = new FuncEntry(); // 存储函数的数据
		Object value = null;
...
//		if (operator == null)
		...
		do {
			...
			if (parser.type == Syntax.OPERATOR_EQUAL_TYPE) {// 如果是=
				...
				if (type == Syntax.KEYWORD_ARRAY_TYPE) {
					...
					if (dimension == 0) {
						setVar(name, interpreteExpression());
	...
	// if (n >= 1) {
	// if (chars[n - 1] != '\\') {
	// inQuotes = !inQuotes; // FIXME 这里可以改为 {inQuotes
	// // = !inQuotes;} 就可以去掉
	// // if(inQuotes)...else
	// }
	// }
